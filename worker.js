...
<body>
  ...
  <div class="wrap">
    ...
    <div class="nav">
      ...
      <button id="navImg" onclick="showImageInfo()">🖼️ Images</button>
    </div>
    <div id="diag"></div>
    <div id="status"></div>
    <div id="view"></div>
  ...
  <script>
    ...
    function albumCard(al,sub){ return '<div class="tile album" onclick="openAlbum('+al.id+')">'+coverHtml(albName(al),{img:albImg(al)})+
    ...
    function openGenre(id){
      ...
      gql("query { genre(where:{id:"+Number(id)+"}) { id enName heName albums { id enName heName images { cdnSmall cdnMedium medium small } tracks { id } } } }").then(function(d){
    ...
    function renderAlbum(id, al){
      ...
      setView(back+'<div class="album-hero">'+coverHtml(albName(al),{img:albImg(al)})+
    ...
    function playAlbum(albumId, index){
      ...
      var artistName=(al.artists&&al.artists.length)?artNames(al.artists):(window._curArtist||"");
      var aimg=albImg(al);
      queue=tracks.map(function(t){ return {id:t.id, file:t.file||"", title:trackName(t), artist:artistName, cover:albName(al), coverImg:aimg}; });
      playIndex(index);
    }
    ...
    function showImageInfo(){
      diagBox('<span class="spinner"></span>Reading image format…');
      gql('query { t: __type(name:"Images"){ fields { name type { kind name ofType { kind name } } } } }').then(function(d){
        var fs=(d.t&&d.t.fields)||[];
        var scalar=fs.filter(function(f){ var t=f.type||{}; return t.kind==="SCALAR" || (t.kind==="NON_NULL" && t.ofType && t.ofType.kind==="SCALAR"); }).map(function(f){return f.name;});
        var sub = scalar.length ? "images { "+scalar.join(" ")+" }" : "";
        var q="query { artists(take:2){ id enName image "+sub+" } playlists(take:2){ id name image cdnImage } genres(take:1){ id enName images } tracks(take:1){ id enName images } albums(take:2){ id enName "+sub+" } }";
        return gql(q).then(function(dd){
          function j(x){ return JSON.stringify(x); }
          var out="Images fields (scalar): "+scalar.join(", ")+"\\n\\n";
          out+="artist.image = "+j(dd.artists&&dd.artists[0]&&dd.artists[0].image)+"\\n";
          out+="artist.images = "+j(dd.artists&&dd.artists[0]&&dd.artists[0].images)+"\\n";
          out+="playlist.image = "+j(dd.playlists&&dd.playlists[0]&&dd.playlists[0].image)+"\\n";
          out+="playlist.cdnImage = "+j(dd.playlists&&dd.playlists[0]&&dd.playlists[0].cdnImage)+"\\n";
          out+="genre.images = "+j(dd.genres&&dd.genres[0]&&dd.genres[0].images)+"\\n";
          out+="track.images = "+j(dd.tracks&&dd.tracks[0]&&dd.tracks[0].images)+"\\n";
          out+="album.images = "+j(dd.albums&&dd.albums[0]&&dd.albums[0].images)+"\\n";
          window._schemaText=out;
          diagBox('<b>Image format</b><pre dir="ltr" style="white-space:pre-wrap;word-break:break-all;font-size:.78rem;margin-top:8px">'+esc(out)+'</pre><button class="btn sm" onclick="copySchema(this)">📋 Copy</button><div class="muted" style="margin-top:8px">Tap Copy and paste it to me.</div>');
        });
      }).catch(function(e){ if(e.message==="login")return; diagBox("❌ "+esc(e.message||e)); });
    }
    ...
    // expose
    window.showSchema=showSchema; window.copySchema=copySchema; window.showImageInfo=showImageInfo;
