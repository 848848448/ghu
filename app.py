...
def _render_gql_type(t):
    if not t:
        return "?"
    if t.get("kind") == "NON_NULL":
        return _render_gql_type(t.get("ofType")) + "!"
    if t.get("kind") == "LIST":
        return "[" + _render_gql_type(t.get("ofType")) + "]"
    return t.get("name") or "?"


@app.route("/api/schema/full")
def api_schema_full():
    """Full structure export: the COMPLETE schema as one readable text blob."""
    if not API_URL:
        return jsonify({"error": "Server not configured."}), 400
    q = (
        "query { __schema { queryType { name } mutationType { name } types { "
        "kind name "
        "fields(includeDeprecated: true) { name args { name type { " + _TYPE_REF + " } } "
        "type { " + _TYPE_REF + " } } "
    ...
    try:
        ...
        def internal(n):
            return (not n) or n.startswith("__")

        scalar_skip = {"String", "Int", "Float", "Boolean", "ID", "DateTime",
                       "JSON", "Json", "Decimal", "BigInt", "Upload"}
        lines = ["ZING API — FULL STRUCTURE",
                 "Generated by the app for rebuilding the whole system.", ""]
        ...
        qfields = root.get("fields") or []
        lines.append("==================== TOP-LEVEL QUERIES (%d) ====================" % len(qfields))
        for f in qfields:
            args = ", ".join(a["name"] + ": " + _render_gql_type(a.get("type")) for a in (f.get("args") or []))
            lines.append("- " + f["name"] + (("(" + args + ")") if args else "") + " -> " + _render_gql_type(f.get("type")))
        lines.append("")

        obj_types = [t for t in types if t.get("kind") == "OBJECT" and not internal(t.get("name"))
                     and t.get("name") not in (root_name, mut_name)]
        lines.append("==================== TYPES (%d) ====================" % len(obj_types))
        for t in obj_types:
            lines.append(t["name"] + " {")
            for f in (t.get("fields") or []):
                args = ", ".join(a["name"] + ": " + _render_gql_type(a.get("type")) for a in (f.get("args") or []) if a.get("name"))
                lines.append("  " + f["name"] + (("(" + args + ")") if args else "") + ": " + _render_gql_type(f.get("type")))
            lines.append("}")
        lines.append("")

        input_types = [t for t in types if t.get("kind") == "INPUT_OBJECT" and not internal(t.get("name"))]
        lines.append("==================== INPUT / FILTER TYPES (%d) ====================" % len(input_types))
        for t in input_types:
            fs = ", ".join(f["name"] + ": " + _render_gql_type(f.get("type")) for f in (t.get("inputFields") or []))
            lines.append(t["name"] + " { " + fs + " }")
        lines.append("")

        enums = [t for t in types if t.get("kind") == "ENUM" and not internal(t.get("name")) and t.get("name") not in scalar_skip]
        lines.append("==================== ENUMS (%d) ====================" % len(enums))
        for t in enums:
            lines.append(t["name"] + " = " + " | ".join(v["name"] for v in (t.get("enumValues") or [])))
        lines.append("")

        if mut_name and by_name.get(mut_name):
            mfs = by_name[mut_name].get("fields") or []
            lines.append("==================== MUTATIONS (%d) ====================" % len(mfs))
            for f in mfs:
                args = ", ".join(a["name"] + ": " + _render_gql_type(a.get("type")) for a in (f.get("args") or []))
                lines.append("- " + f["name"] + (("(" + args + ")") if args else "") + " -> " + _render_gql_type(f.get("type")))
            lines.append("")
        ...
        text = "\n".join(lines)
        return jsonify({"text": text, "counts": {
            "queries": len(qfields), "types": len(obj_types),
            "inputs": len(input_types), "enums": len(enums),
            "mutations": len(by_name.get(mut_name, {}).get("fields") or []) if mut_name else 0,
        }})
    except Exception as exc:  # noqa: BLE001
        return jsonify({"error": str(exc)}), 502
